...
import { UmilanError } from '../utils/errors.js';

const POLL_INTERVAL = 2000;
const POLL_TIMEOUT  = 120_000;

export class ImageModule {
  /** @param {import('../client.js').UmilanClient} client */
  constructor(client) {
    this._client = client;
  }

  /**
   * Generate an image from a text prompt using umilan.v1.
  ...
  async _poll(jobId, onProgress) {
    ...
    while (Date.now() < deadline) {
      ...
      if (status.status === 'failed') {
        throw new UmilanError(status.error ?? 'Image generation failed');
      }
    }

    throw new UmilanError('Image generation timed out');
  }
