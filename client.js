import { UmilanError, RateLimitError, AuthError, NetworkError } from './utils/errors.js';
...
const DEFAULT_BASE_URL = 'https://evilan.onrender.com';
const DEFAULT_TIMEOUT  = 30_000;
const DEFAULT_RETRIES  = 3;

export class UmilanClient {
  /**
    ...
    while (attempt <= this.retries) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new UmilanError(body.error ?? `HTTP ${res.status}`, res.status);
        }

        return await res.json();
      } catch (err) {
        clearTimeout(timer);
        if (err instanceof UmilanError) throw err;
